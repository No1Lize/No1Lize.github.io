...
def _tracked_entities(track: dict[str, Any]) -> tuple[list[str], list[str], list[str]]:
    ...
    keywords = _unique(track.get("keywords", []), 60)
...
def _fallback_spec(track: dict[str, Any]) -> dict[str, Any]:
...
def generated_wechat_sources(
    ...
    """Generate whitelist-first sources while preserving uncovered tracks."""
    ...
    for track in tracks:
        ...
        if not sector:
            continue
        configured = _configured_for_sector(registry, sector)
        if configured:
            sources.extend(
                _configured_spec(account, sector, indexed[sector.casefold()], settings)
                for account in configured
            )
        else:
            sources.append(_fallback_spec(track))
    return sources[:80]
