...
def _reconcile_eastmoney_retention_accounting(
    ...
    The rolling crawler records ``newAccepted`` and ``retainedPreviousCount``.
    Retention removes the oldest rows first, so retained history is consumed
    before current-run rows whenever the snapshot must shrink after a rebase.
    """
    ...
    for raw in raw_statuses:
        ...
        if has_history_accounting:
            # The newest current-run rows survive before retained historical
            # rows under the formal newest-first policy.
